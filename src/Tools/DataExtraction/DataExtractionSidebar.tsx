...
const DataExtractionSidebar: React.FC = () => {
            ...
            bookPdfsData = await Promise.all(
              booksData
                ...
                .map(async (book: { _id: string; bookName: string; previewUrl: string }) => {
                  ...
                  try {
                    if (book.previewUrl) {
                      fileUrl = await fetchBookFile(book.previewUrl);
                    }
                  } catch (err) {
  ...
  return (
    <div className="h-full test-center p-6 pt-10 sm:pt-24 md:pt-10 overflow-y-auto bg-gradient-to-b from-white to-gray-50 shadow-xl rounded-2xl w-full max-w-xs border border-gray-200">
      {/* Sticky Header */}
      ...
      <div className="mb-8">
        ...
        {collections.length > 0 ? (
          collections.map((collection) => (
            <div
              key={collection._id}
              className={`mb-3 rounded-lg transition-all ${expandedCollection === collection._id
            ...
            >
              ...
              >
                <span
                  className={`mr-2 text-gray-500 transform transition-transform ${expandedCollection === collection._id ? "rotate-90" : ""
                    }`}
                >
                  ▶
                </span>
